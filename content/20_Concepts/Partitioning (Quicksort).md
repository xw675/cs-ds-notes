...
**Context:** [[FIT2004_MOC]] · the engine inside [[Quick Sort]] and [[Quickselect]] — promoted out of [[Quick Sort]] because W3 examines the **schemes themselves**
...
- **The payoff** ➔ the pivot is provably final ⟹ both recursive calls skip it. [[Merge Sort]] has no equivalent free element; this is where quicksort's "trivial combine" comes from.
- **Cost floor is identical** ➔ every scheme touches all $N$ items once ⟹ $\Theta(N)$ **comparisons** per call. Schemes differ only in **writes** and **space** — so a "which partition is better" question is always a swap-count or memory question ➔ [[Sorting Problem]].

### 2. Out-of-place Partitioning *(the unit notes' “naive 3-way partitioning”, §3.2)*
- **Mechanism** ➔ scan left→right appending each item to a `left` buffer ($<p$), an `equal` buffer ($=p$) and a `right` buffer ($>p$), then concatenate and copy back over the original range.
- **Two buckets vs three** ➔ the two-bucket form ($\le p$ / $>p$) is the one the lecture calls unstable; the unit notes' **naive 3-way** form keeps the $=p$ bucket, which restores stability **and** groups the duplicates so the recursion can skip them.
...
- **Why it is still NOT stable** ➔ `≤ pivot` sends every item **equal to the pivot** into the left buffer, i.e. **in front of** the pivot even when it started behind it ⟹ the pivot itself reorders its own duplicates.
- **The stable repair** ➔ keep a **third** buffer for `== pivot` and concatenate $<p$ + $=p$ + $>p$ ⟹ stable. Lecturer's framing: *anything can be made stable with more memory* ➔ [[Sorting Problem]] §6.

### 3. Hoare's In-place Partitioning *(the version taught in this unit)*
- **Setup** ➔ swap the pivot to the **front** of the range; set `L_bad` to the second slot and `R_bad` to the last.
...
- **Finish** ➔ swap the parked pivot into `R_bad` ⟹ under this variant `R_bad` **is the pivot's final index**, exactly like Lomuto's return value.
...
- **Stability** ➔ the pointer conditions **can** be tightened into a stable rule using the pivot's original index plus some arithmetic — but the **final swap** of the pivot from the front to `R_bad` jumps it over every item in between and destroys it ⟹ **not stable**.
- **Lecturer-flagged fragility** ➔ the naive write-up has edge cases where the final pivot swap must **not** fire, and a further special case that fails without an extra guard. Hoare's is the scheme you get wrong; Lomuto's is the scheme you get right.
...
- **Verdict from the lecture** ➔ in place · **writes far more** — every item below the pivot is swapped even when it was already on the correct side, where Hoare's only touches genuinely misplaced pairs · **still unstable** ⟹ *"worse than Hoare's"*.
- **Why it survives anyway** ➔ easier to understand, easier to implement, and free of Hoare's edge cases — this is the FIT1008 version, and the one to write under time pressure.
...
- **Invariants** ➔ `[1 … boundary1-1]` is $<p$ · `[boundary1 … j-1]` is $=p$ · `[boundary2+1 … N]` is $>p$ · `[j … boundary2]` is **unprocessed**, and is empty exactly when the loop exits at `j > boundary2`.
...
- **The problem** ➔ given $k\le n$ pivots $p_{1}<p_{2}<\dots<p_{k}$ (**not** supplied in sorted order), rearrange so every item $\le p_{1}$ comes first, then everything above $p_{1}$ and at most $p_{2}$, and so on — ordinary partitioning is the case $k=1$.
- **Step 0 always** ➔ sort the pivots, $\Theta(k\log k)$; dominated by everything below because $n\ge k$.
- **Naive: $\Theta(nk)$** ➔ 2-way partition on $p_{1}$, then 2-way partition the **suffix after $p_{1}$** on $p_{2}$, and so on ➔ $k$ calls of up to $\Theta(n)$ each.
- **Divide and conquer: $\Theta(n\log k)$** ➔ partition the whole array on the **middle** pivot $p_{\lceil k/2\rceil}$, then recurse left with the lower $k/2$ pivots and right with the upper $k/2$ ➔ $\Theta(\log k)$ levels $\times$ $\Theta(n)$ per level ([[Divide and Conquer]]).
- **Why it is optimal — prove by reduction** ➔ take all $n$ elements as pivots ($k=n$); $k$-partitioning then **sorts** the array, so beating $O(n\log k)$ would beat the comparison model's $\Omega(n\log n)$ floor ➔ $\Omega(n\log k)$ ([[Sorting Problem]]).
- **The transferable move** ➔ identical in shape to [[K-way Merge]]'s $\Theta(Nk)\to\Theta(N\log k)$ upgrade: a **linear sweep over $k$ things** becomes **logarithmic in $k$** once a balanced structure is imposed on them — D&C here, a min-[[Heap]] there.
...
> ```
> 💡 **Common Mistake:** **Advancing `j` on the red branch** ➔ the item swapped in from `boundary2` is **unexamined**; incrementing `j` skips it and leaves a $<p$ or $>p$ element stranded in the $=p$ block.
...
> ```
> 💡 **Common Mistake:** **Recursing on the pivots but not narrowing the array range** ➔ each call must inherit only its own side; re-partitioning the full array at every level gives $\Theta(nk)$ back and loses the whole point.
...
> [!NOTE] **When It Flips:** integer keys ⟹ the swap-count gap between Hoare's and Lomuto's is invisible, so write Lomuto's. Large records, or a stated write/IO cost ⟹ Hoare's $3\times$ write saving is the whole answer. The moment the key domain is small (grades, flags, three sensor states) the argument stops being about swaps at all and the 3-way split changes the **complexity class** ➔ [[Quick Sort]] §3.

## 📊 Exam Execution Trace

### Manual Execution Trace
Hoare's on the lecture's array `[2, 8, 6, 4, 1, 7, 3, 5]`, pivot $=4$, **1-indexed**. Step $0$ parks the pivot at the front.
...
**Read the result:** $4$ sits at index $4$ — its **final** sorted position — after only $3$ swaps for $8$ items, and every moved item moved exactly once. Lomuto's on the same input writes every one of the three items below the pivot plus the two pivot swaps.

### Manual Execution Trace *(prep W4 — the same array, both schemes)*
`[7, 1, 12, 9, 3, 3, 10, 6, 7, 14, 4]`, pivot $=7$ (the **first** element).

**Naive 3-way (out-of-place).** One pass builds three buckets, then concatenates.

| Bucket | Contents (input order preserved) |
| :--- | :--- |
| $<7$ | `[1, 3, 3, 6, 4]` |
| $=7$ | `[7, 7]` |
| $>7$ | `[12, 9, 10, 14]` |

**Result:** `[1, 3, 3, 6, 4, 7, 7, 12, 9, 10, 14]` — the pivot's duplicates sit **together**, so quicksort can recurse on the two outer blocks only.

**Hoare's, in-place.** `lo` advances until it meets a value $>7$; `hi` retreats until it meets a value $\le 7$; swap; stop when they cross; finally swap the parked pivot with `hi`.
...
**Read the result:** the pivot $7$ lands at index $7$, its final position, in $4$ swaps. But the **duplicate** $7$ (originally index $9$) now sits at index $4$ — *left* of the pivot and not adjacent to it. Naive 3-way grouped the duplicates; Hoare's scatters them, and that is precisely the gap the **Dutch national flag** scheme was designed to close.

- **Convention warning** ➔ the prep sheet's `hi` stops on $\le p$ while §3's `R_bad` stops on $<p$; both are correct schemes but they produce **different arrays**. State which stopping rule you are using before the first swap.
...
- 💡 **Claiming a partition is $\Theta(N)$ "swaps"** ➔ it is $\Theta(N)$ **comparisons**; swaps are $O(N)$ but the constant is the entire point of the comparison between schemes.
...
> > - **Why:** **One swap repairs two items** ➔ Hoare's only touches genuinely misplaced pairs, giving $\approx3\times$ fewer writes. This is invisible for integer keys and decisive for large records or an IO-costed model ➔ [[Sorting Problem]].
...
> > - **Why:** **The pivot reorders its own duplicates** ➔ a two-bucket split has nowhere else to put them. A **third** buffer for `== pivot` restores stability at more memory — the lecturer's general point that *anything* can be made stable with space ➔ [[Sorting Problem]] §6.
...
> > [!SUCCESS]- Answer
> > - **Short answer:** `[1 … boundary1-1]` $<p$ · `[boundary1 … j-1]` $=p$ · `[boundary2+1 … N]` $>p$ · `[j … boundary2]` **unprocessed**; quicksort then recurses on the $<p$ and $>p$ regions **only**.
> > - **Why:** **The $=p$ block is already final** ➔ it needs no further sorting, so with $d$ distinct keys the recursion depth is $O(\log d)$ and an all-equal input costs $\Theta(N)$ instead of $\Theta(N^{2})$ ➔ [[Quick Sort]].
...
> > - **Why:** **D&C balances the pivot budget, not the data** ➔ each level halves the number of *pivots* still to place while still touching $\Theta(n)$ items, giving $\Theta(\log k)$ levels. **The lower bound comes from a reduction** ➔ set $k=n$ and $k$-partitioning becomes sorting, so an $o(n\log k)$ algorithm would break $\Omega(n\log n)$ ([[Sorting Problem]]). **Sorting the pivots first is free** ➔ $\Theta(k\log k)$ is dominated because $k\le n$.
