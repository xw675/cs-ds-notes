...
> - **🎯 Objective:** merge $k$ sorted lists holding $N$ items into one ➔ each step asks only **"which head is smallest?"**, so the cost of answering it *is* the algorithm.
...
- **Two lists ⟹ one comparison** ➔ [[Merge Sort]]'s merge compares `a[ia]` against `b[ib]` — $\Theta(1)$ per output item.
- **$k$ lists ⟹ a minimum query** ➔ "smaller of two" becomes "**minimum of $k$**", which a linear scan answers in $\Theta(k)$ ⟹ $\Theta(Nk)$ total. **Finding the minimum is the bottleneck.**
...
- **Loop** ➔ `get_min` emits the next output item in $\Theta(\log k)$; then `add` that list's **successor**, keeping the heap at size $k$.
- **Total** ➔ $N$ items × (one extract + one insert) ⟹ $\Theta(N\log k)$ time, $\Theta(k)$ auxiliary beyond the output.
- **Carrying the list id is mandatory** ➔ without it you know *what* the minimum was but not *which list to refill from*; the index field makes the successor lookup $O(1)$.
...
- **Termination** ➔ each iteration removes one item from a finite total of $N$ and refills only from a strictly advancing index.
...
- **The reduction** ➔ split any sequence of length $N$ into $N$ lists of length $1$ (each **trivially sorted**) and merge them with $k=N$. The output is the sorted sequence ⟹ the merge algorithm **is** a comparison sort, and beating $O(N\log k)=O(N\log N)$ would break the $\Omega(N\log N)$ floor ➔ [[Sorting Problem]].
- **Read the shape, not just the result** ➔ a **lower bound by reduction**: you inherit a known bound by showing the new problem can *simulate* the old one. The same move proves optimality claims across the unit ➔ LO1.
- **The bound is on the comparison model only** ➔ a non-comparison merge over bounded integer keys is not covered, exactly as [[Counting Sort]] escapes the sorting floor.
...
| Divide & conquer (recursive halving) | $\Theta(N\log k)$ | $\Theta(N)$ scratch | lists already resident and you want [[Merge Sort]]'s 2-way merge **reused unchanged** |
...
> [!NOTE] **When It Flips:** the heap wins once $\log_2 k < k$ in the constants, i.e. from roughly $k\ge8$. Note $\Theta(N\log k)$ **is** [[Merge Sort]] when $k=N$: seeding $N$ singleton lists reproduces $\Theta(N\log N)$ exactly — the two algorithms are the same recurrence read from opposite ends.

## 📊 Exam Execution Trace & Applied Exercises

### Manual Execution Trace
$k=3$: $A=[1,4,9]$, $B=[2,3,8]$, $C=[5,6,7]$. Heap shown as sorted contents; only the **root** is actually ordered.
...
$$
\begin{aligned}
T_{\text{scan}} &= N\cdot\Theta(k) = \Theta(10^{9}) \text{ comparisons} \\
T_{\text{heap}} &= N\cdot\Theta(\log_2 k) = 10^{6}\times 10 = \Theta(10^{7}) \\
S_{\text{aux}} &= \Theta(k) = \Theta(10^{3}) \text{ triples, independent of } N
\end{aligned}
$$
**Final Extracted Output:** a $100\times$ reduction with auxiliary space unchanged at $\Theta(k)$. That $\Theta(k)$ bound is what permits $N$ to exceed memory: only $k$ records are ever resident, and the lists can be read as disk streams because the algorithm never looks back — a strategy loading all $N$ items cannot merge data larger than memory at any complexity ➔ [[Algorithmic Complexity]].

## ⚠️ Common Mistakes
- 💡 **Quoting $\Theta(N\log N)$ for the heap version** ➔ the heap never holds more than $k$ items, so each operation is $\Theta(\log k)$ — the whole point is that $k\ll N$.
- 💡 **Losing the list id** ➔ storing bare values makes the refill ambiguous; the triple is not optional bookkeeping, it is what keeps the refill $O(1)$.
...
> > - **Why:** **Heads dominate their own lists** ➔ sortedness makes each list's unconsumed head that list's minimum; the heap holds all $k$ heads, so the global minimum must be among them, and the root *is* their minimum ➔ [[Invariant]].
...
> > - **Why:** **Lower bound by reduction** ➔ every singleton list is trivially sorted, so it is a legal input to the merge, and its output is the sorted sequence. Any faster merge would sort faster than the comparison model allows ➔ [[Sorting Problem]]. This bounds the **comparison** model only.
