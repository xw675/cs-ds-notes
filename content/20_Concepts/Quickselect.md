...
- **Specification** ➔ given $N$ unsorted orderable items and a rank $k$, return the element that **would** sit at index $k$ if the list were sorted — without producing the sorted list. Equivalently, the lecture's phrasing: return the $k$ **smallest** items, whose internal order does not matter.
- **Worked reading** ➔ on `21, 84, 16, 14, 79, 51, 66, 21, 54, 32`: $k=1$ ➔ `14` · $k=2$ ➔ `{14, 16}` · $k=5$ ➔ `{14, 16, 21, 21, 32}`.
- **The whole quartile family is one call** ➔ $Q_1$ at $k=N/4$ · **median** at $k=N/2$ · $Q_3$ at $k=3N/4$ ➔ [[Measures of Spread and Boxplots]].
...
- **The median is why it matters** ➔ it is the pivot [[Quick Sort]] wants, which is what makes selection worth a dedicated algorithm rather than a corollary of sorting.

### 2. The Baseline — Sort and Slice
- **The obvious algorithm** ➔ sort, then slice the first $k$ ⟹ $O(NM\log N) + O(k)$, where $M$ is the **cost of one comparison** (integers $O(1)$, $k$-character strings $O(k)$) ➔ [[Sorting Problem]].
- **The linear sorts do not rescue it** ➔ [[Counting Sort]] needs the key range capped and `item_max` may be huge; [[Radix Sort]] needs bounded key width — neither can be **assumed** for arbitrary input, so the $\log N$ stands.
...
- **Why the cost collapses** ➔ [[Quick Sort]] recurses on **both** halves so every level still costs $\Theta(N)$ across $\log N$ levels; quickselect recurses on **one**, so the level costs **halve** — a [[Geometric Series]] with $r=\tfrac12$ that sums to $<2N$.

### 4. Cost Profile
- **Best case is $\Theta(N)$, not $\Theta(1)$** ➔ even a first-try hit ($j=k$ immediately) has already paid for **one full partition** ⟹ the whole input is read no matter what.
- **Expected $\Theta(N)$** ➔ $T(N)=T(N/2)+\Theta(N)$ under balanced pivots ⟹ root-dominated regime ($r=a/b=\tfrac12<1$) ➔ [[Solving Recurrences (Telescoping)]]. *(The lecturer's formal average-case derivation — multiply by $N$, subtract the $(N{-}1)$ instance so the summations cancel, solve — is flagged **NOT EXAMINABLE**; it lands on $\Theta(N)$ because both surviving fractions are $<1$. The same trick appears in [[Quick Sort]]'s non-examinable average case.)*
...
- **The tempting plan** ➔ call quickselect for the exact median, use it as the pivot, and skip quicksort's own partition entirely — quickselect **already partitioned** the range, so that step is free.
- **It does not work** ➔ quickselect's own worst case is $\Theta(N^{2})$ (pivot $\ne k$ until the last iteration), so the pivot-finding call at the root costs $N^{2}$, and the level sum is $N^{2}+\tfrac{N^{2}}{2}+\tfrac{N^{2}}{4}+\dots = 2N^{2}$ ⟹ the sort stays $\Theta(N^{2})$. *Derivation in the trace section below.*
- **The fix is to stop demanding the exact median** ➔ any pivot inside the middle band is enough, because a **constant-fraction** split already gives logarithmic depth ➔ [[Quick Sort]] §3.
- **[[Median of Medians]] delivers exactly that band** ➔ a pivot provably between the $30$th and $70$th percentile, found in $\Theta(N)$ **worst case** ⟹ height $O(\log N)$, partition already done ⟹ $\Theta(N\log N)$ worst-case quicksort.
- **Lecturer's practical coda** ➔ *"in reality, random pivot works well due to probability"* — the guarantee is bought with a constant factor nobody pays voluntarily.

### 6. Adapting Quickselect to a New Problem *(applied W4 — LO1)*
- **The reusable skeleton** ➔ *partition once ➔ read something off the pivot's position ➔ recurse into the single side that can still contain the answer.* Every variant below changes only **what is read off** and **which side survives**.
- **Weighted median** ➔ each $a_{i}$ carries weight $w_{i}$ with $\sum w_{i}=1$; the answer is the $a_{k}$ whose left and right weight sums are each $\le\tfrac12$. Swap "compare rank $j$ to $k$" for "compare **weight sums** to $w$": if neither side exceeds $w$ the pivot **is** the answer, otherwise recurse into the heavier side with $w$ reduced by the weight you skipped. Same $\Theta(n)$ expected.
- **The $k$ closest to the median** ➔ run quickselect once for the median $m$, then run it again for the $k$-th smallest $\lvert a_{i}-m\rvert$; take everything strictly below that value and **top up** to $k$ from the ties. Two linear calls ⟹ $\Theta(n)$.
- **Why the top-up is mandatory** ➔ the elements are unique but their **distances** are not ($m+d$ and $m-d$ tie), so "strictly closer than the $k$-th" can yield fewer than $k$ items; any tied element will do, since they are equally close.
- **Killing the $\Theta(n)$ space** ➔ do **not** materialise the shifted/absolute copy; run a modified quickselect that *interprets* `array[i]` as $\lvert \text{array}[i]-m\rvert$ on the fly ⟹ $\Theta(n)$ time, $O(1)$ extra space beyond the output.
- **Order statistic across two sorted arrays** `[D]` ➔ $\text{order}(a[i])=i+\lvert\{j:b[j]<a[i]\}\rvert$ is **monotone in $i$**, hence binary-searchable: an inner [[Binary Search]] on $b$ inside an outer one on $a$ gives $\Theta(\log n\log m)$; running the **two searches simultaneously** — comparing $a[i]$ with $b[j]$ and moving one bound per step — removes the nesting for $\Theta(\log n+\log m)$.
...
>                                     w - (left + weight[j]))
> ```
> 💡 **Common Mistake:** **Partitioning the values without the weights** ➔ the weight array must be permuted by the *same* swaps, or every weight is attached to the wrong element after the first call.
> 💡 **Common Mistake:** **Recursing right with the original $w$** ➔ the weight already discarded on the left must be **subtracted**, exactly as $k$ would be re-indexed in a relative-index quickselect.
...
> [!NOTE] **When It Flips:** quickselect wins while exactly one rank is needed from a resident array. Ask for $\ge\log N$ different ranks and sorting once amortises better; drop the "resident array" assumption and quickselect is unusable at any cost, because it must partition the whole input — that regime belongs to the size-$k$ heap in [[Online Algorithm]] (Tutorial 03 Q8).

## 📊 Exam Execution Trace & Applied Exercises

### Manual Execution Trace
`quickselect([7, 2, 9, 1, 5, 8, 3], k=2)` — the $3$rd smallest. Pivot $=$ middle element of the live window (Lomuto, as in [[Partitioning (Quicksort)]]).
...
$$
\begin{aligned}
\text{cost of finding the median at the root} &= \Theta(N^{2}) &&\text{(quickselect's worst case)} \\
\text{level } 0 &= N^{2} \\
\text{level } 1 = 2\left(\tfrac{N}{2}\right)^{2} &= \tfrac{N^{2}}{2}, \qquad
\text{level } 2 = 4\left(\tfrac{N}{4}\right)^{2} = \tfrac{N^{2}}{4} \\
\text{level } i &= \frac{N^{2}}{2^{i}} \\
T(N) = \sum_{i\ge0}\frac{N^{2}}{2^{i}} &= N^{2}\left(1+\tfrac12+\tfrac14+\dots\right) = 2N^{2} = \Theta(N^{2})
\end{aligned}
$$
**Final Extracted Output:** the split is now **perfect** at every level and the sort is *still* quadratic — the cost moved from the recursion shape into the **pivot-finding step**. Only a pivot rule that is linear in the **worst** case ([[Median of Medians]]) closes it, and a merely $30/70$ pivot suffices because constant-fraction splits already give $O(\log N)$ depth.

### Applied Exercise 3 — the probability argument for expected $\Theta(n)$
**Problem:** call a pivot **good** if it falls in the middle $50\%$ of the sorted sequence. Bound quickselect's expected work.
$$
\begin{aligned}
\text{worst good pivot} &: \text{the 25th or 75th percentile} \Rightarrow \text{recurse on } 0.75n \\
T(n) &= cn + 0.75cn + 0.75^{2}cn + \dots \\
&= cn\sum_{i\ge0}0.75^{i} \;\le\; \frac{1}{1-0.75}\cdot cn \;=\; 4cn \\
\Pr[\text{good pivot}] &= \tfrac12 \Rightarrow \mathbb{E}[\text{flips to first good}] = \tfrac{1}{1/2} = 2 \\
\mathbb{E}[T(n)] &\le 2\cdot 4cn = 8cn = O(n)
\end{aligned}
$$
**Final Extracted Output:** $O(n)$ expected. The two moves to reproduce are (i) a **constant-fraction** shrink turns the level costs into a [[Geometric Series]] that sums to a constant multiple of $n$, and (ii) a pivot that is good with probability $p$ costs $1/p$ attempts in expectation, so the whole bound is multiplied by a **constant**. The identical argument with both halves kept gives [[Quick Sort]]'s $O(n\log n)$.
...
- 💡 **Claiming a best case better than $\Theta(N)$** ➔ hitting $j=k$ on the first try still costs one full partition; the input must be read.
...
> [!FAQ]- Use quickselect to find quicksort's exact median pivot. What is the worst-case complexity, and why is the answer surprising?
> - **Hint:** The split is perfect. Price the pivot, not the recursion.
> > [!SUCCESS]- Answer
> > - **Short answer:** Still $\Theta(N^{2})$ — the level sum is $N^{2}+\tfrac{N^{2}}{2}+\tfrac{N^{2}}{4}+\dots=2N^{2}$.
> > - **Why:** **The cost moved, it did not vanish** ➔ quickselect's own worst case is quadratic, so a perfectly balanced recursion tree is fed by a quadratic pivot search at every node. The repair is to stop insisting on the *exact* median: a $30/70$ pivot from [[Median of Medians]] costs $\Theta(N)$ worst case and constant-fraction splits already give $O(\log N)$ depth ⟹ $\Theta(N\log N)$.
...
> > - **Why:** **Distances need not be unique even when the values are** ➔ $m+d$ and $m-d$ both sit at distance $d$, so the strict set can be short of $k$; any tied element is equally close, so the top-up is free. **Two $\Theta(n)$ calls are still $\Theta(n)$** ➔ constants do not change the class, and [[Median of Medians]] makes both calls worst-case linear. **Do not build the transformed array** ➔ interpret `array[i]` as its absolute difference from $m$ inside the partition and the $\Theta(n)$ auxiliary space disappears.
